...
const Header = () => {
    ...
    const defaultUser = {
        ...
        role: 'guest',
        profileImage: '/assets/images/user-profile.jpeg'
    };
    ...
    const getUserName = () => {
        if (!user || !user.name) return 'Guest User';
        return user.name.length > 15 ? `${user.name.substring(0, 15)}...` : user.name;
    };
    
    // Format email for display
    const getUserEmail = () => {
        if (!user || !user.email) return 'guest@example.com';
    ...
    const getRoleBadge = () => {
        ...
        switch (role) {
            ...
            default:
                ...
                badgeText = role.charAt(0).toUpperCase() + role.slice(1);
        }
        
        return (
            <span className={`text-xs rounded px-2 py-0.5 ${badgeClass} ltr:ml-2 rtl:mr-2`}>
                {badgeText}
            </span>
        );
    };
        ...
        <header className={`z-40 ${themeConfig.semidark && themeConfig.menu === 'horizontal' ? 'dark' : ''}`}>
            ...
            <Dropdown
                ...
                button={
                    <div className="relative">
                        <img 
                            className="w-9 h-9 rounded-full object-cover saturate-50 group-hover:saturate-100 border-2 border-transparent group-hover:border-primary" 
                        ...
                        <div className="absolute bottom-0 right-0 w-3 h-3 rounded-full border-2 border-white dark:border-dark bg-success"></div>
                    </div>
                }
            >
                <ul className="text-dark dark:text-white-dark !py-0 w-[230px] font-semibold dark:text-white-light/90">
                    {/* User info section */}
                    <li className="border-b border-white-light dark:border-white-light/10">
                        <div className="flex items-center px-4 py-4">
                            ...
                            <div className="ltr:pl-4 rtl:pr-4 truncate">
                                ...
                                <div className="text-black/60 dark:text-dark-light/60 truncate text-sm">
                                    {getUserEmail()}
                                </div>
                    ...
                    {/* Menu items */}
                    <li>
                        <Link to="/users/profile" className="dark:hover:text-white hover:bg-primary/10 flex items-center px-4 py-2.5">
                    ...
                    {user.role === 'admin' && (
                        <li>
                            <Link to="/admin/dashboard" className="dark:hover:text-white hover:bg-primary/10 flex items-center px-4 py-2.5">
                    ...
                    <li>
                        <Link to="/apps/mailbox" className="dark:hover:text-white hover:bg-primary/10 flex items-center px-4 py-2.5">
                            <IconMail className="w-4.5 h-4.5 ltr:mr-2 rtl:ml-2 shrink-0" />
                            <span>Inbox</span>
                            <span className="ltr:ml-auto rtl:mr-auto bg-primary text-white text-xs px-2 py-0.5 rounded-full">3</span>
                        </Link>
                    </li>
                    
                    <li>
                        <Link to="/auth/boxed-lockscreen" className="dark:hover:text-white hover:bg-primary/10 flex items-center px-4 py-2.5">
                    ...
                    {/* Divider */}
                    <li className="border-t border-white-light dark:border-white-light/10 mt-1"></li>
                    
                    {/* Logout */}
                    <li>
                        <button 
                            onClick={handleLogout}
                            className="text-danger hover:bg-danger/10 hover:text-danger-dark w-full text-left flex items-center px-4 py-2.5"
                        >
